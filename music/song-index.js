...
function customSlug(title) {
    return title.toLowerCase().replace(/[()?,!.]/g, "") // Usuwamy interpunkcję i nawiasy
        .trim().split(/\s+/) // Dzielimy na słowa
        .map(word => {
            // Jeśli słowo zawiera cyrylicę (zakres \u0400-\u04FF)
            if (/[\u0400-\u04FF]/.test(word)) {
                return encodeURIComponent(word).replace(/%/g, "") // Usuwamy % (zostaje czysty hex)
                    .toLowerCase();
            }
...
const getTrackDetails = async () => {
    ...
    try {
        ...
        const ImageDisplay = attr.artwork?.url?.replace('{w}', '250').replace('{h}', '250') || 'https://i.ibb.co/1JJGn1Rx/hit_default_plug.png';
