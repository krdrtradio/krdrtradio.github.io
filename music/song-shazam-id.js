...
const getTrackShazamDetails = async () => {
    ...
    try {
        ...
        const artwork = attr.artwork?.url ? attr.artwork.url.replace('{w}', '250').replace('{h}', '250') : 'https://i.ibb.co/1JJGn1Rx/hit_default_plug.png';
        ...
        const titleHtml = relatedSongId ? `<p class="getSongs_info_title"><a href="song?id=${relatedSongId}">${escapeHTML(attr.title)}</a></p>` : `<p class="getSongs_info_title">${escapeHTML(attr.title)}</p>`;
        container.innerHTML = `
