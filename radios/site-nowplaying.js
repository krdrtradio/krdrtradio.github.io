...
function SongToLink(track) {
    return `<a href="https://krdrtradio.github.io/music/song-find?s=${encodeURIComponent(track)}" target="_blank">${track}</a>`;
}
async function getNowPlayingGrupaZPR(stationId) {
    const container = document.getElementById('resultTrack');
    const url = `https://front-api.grupazprmedia.pl/music/v2/now_playing/${stationId}/`;
    try {
        ...
        if (data.current !== null) {
            ...
            container.innerHTML = `<h4>Teraz gramy:</h4><small>${SongToLink(`${artist} - ${title}`)}</small>`;
        } else {
...
function renderProgramGrupaZPR(program) {
    ...
    //     return;
    // }
    const imageDisplay = program.thumbnail_uri ? `<img decoding="async" src="${program.thumbnail_uri}" alt="${escapeHTML(program.name)}">` : '';
    container.innerHTML = `
...
async function getNowPlayingAgora(stationId) {
    ...
    try {
        ...
        if (track && track.artist_name && track.song_title) {
            container.innerHTML = `<h4>Teraz gramy:</h4><small>${SongToLink(`${track.artist_name} - ${track.song_title}`)}</small>`;
        } else {
...
async function getNowPlayingGrupaRMF(stationId) {
    ...
    try {
        ...
        if (utwor) {
            ...
            container.innerHTML = `<h4>Teraz gramy:</h4><small>${SongToLink(tekst)}</small>`; // Aktualnie w RMF FM
...
async function getNowPlayingRadio(stationId) {
    ...
    try {
        ...
        if (data && data.length > 0) {
            ...
            container.innerHTML = `<h4>Teraz gramy:</h4><small>${SongToLink(currentTrack)}</small>`;
        } else {
...
async function getNowPlayingPlaylist(stationId) {
    ...
    try {
        ...
        if (songTitle) {
            container.innerHTML = `<h4>Teraz gramy:</h4><small>${SongToLink(songTitle)}</small>`; // Ostatnio grany utwór:
        } else {
...
async function getNowPlayingOpenFm(stationId) {
    ...
    try {
        ...
        if (currentSong) {
            if (container) container.innerHTML = `<h4>Teraz gramy:</h4><small>${SongToLink(currentSong)}</small>`;
            return currentSong;
        } else {
...
async function getPlanetaFMSong() {
    try {
        ...
        container.innerHTML = `<h4>Teraz gramy:</h4><small>${SongToLink(songs)}</small>`; // Aktualny utwór:
    } catch (error) {
        if (error instanceof TypeError) {
...
async function getTomorrowlandSong(stationId = 'main') {
    ...
    try {
        ...
        if (container) {
            container.innerHTML = `<h4>Teraz gramy:</h4><small>${SongToLink(formatToTitleCase(formattedTrack))}</small>`;
        }
...
async function getNowPlayingRevma(stationId) {
    ...
    try {
        ...
        const result = (artist === null || artist === "") ? title : `${artist} - ${title}`;
        console.log(result);
        if (result === "" || result === null) {
            container.innerHTML = '';
        } else {
            container.innerHTML = `<h4>Teraz gramy:</h4><small>${SongToLink(formatToTitleCase(result))}</small>`;
        }
        return result;
    } catch (error) {
        if (error instanceof TypeError) {
    ...
    return new Promise((resolve, reject) => {
        ...
        eventSource.onmessage = (event) => {
            try {
                ...
                container.innerHTML = `<h4>Teraz gramy:</h4><small>${SongToLink(formatToTitleCase(data.streamTitle))}</small>`;
            } catch (error) {
                eventSource.close();
...
async function getNowPlayingOnlineRadioBox(stationId) {
    try {
        ...
        if (title === "" || title === null) {
            ...
            container.innerHTML = `<h4>Teraz gramy:</h4><small>${SongToLink(formatToTitleCase(title))}</small>`;
        }
    } catch (error) {
        if (error instanceof TypeError) {
            console.error(error);
...
async function getmetadataICY(streamUrl) {
    ...
    try {
        ...
        if (container) {
            ...
            container.querySelector('span').innerHTML = SongToLink(formatTitle(data.playlist_format));
        }
        return data.playlist_format;
    } catch (error) {
        clearContainer();
...
async function getNowPlayingBounce() {
    // Publiczny proxy dodający nagłówki CORS
    const container = document.getElementById('resultTrack');
    const apiUrl = `https://app.bounce.com.pl/api/nowplaying`;
    const proxy = `https://cors.krdrtradio.workers.dev/?url=${encodeURIComponent(apiUrl)}`;
    try {
        const response = await fetch(proxy);
        if (!response.ok) throw new Error('Błąd sieci');
        ...
        if (result === "" || result === null) {
            ...
            container.innerHTML = `<h4>Teraz gramy:</h4><small>${SongToLink(result)}</small>`;
        }
        return result;
    } catch (error) {
        if (error instanceof TypeError) {
