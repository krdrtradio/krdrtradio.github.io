...
function isValidProgramId(id) {
    // Brak ID = program może istnieć tylko w ramówce
    if (!id) return true;
...
function renderCurrent() {
    ...
    const baseHostDataOS = data.only_the_schedule_hosts ? "" : baseHostData;
    const baseHostProgram = Array.isArray(program.host) ? program.host.join(', ') : (program.host || "");
    const baseHosts = program.schedule_only_host ? "" : baseHostDataOS;
...
function renderSchedules() {
    ...
    dayOrder.forEach(day => {
        ...
        scheduleSource.filter(p => {
            ...
            const urlNameP = (urlP) ? `<div class="schedule_program_name"><a href="${urlP}" target="_blank">${displayName}</a></div>` : `<div class="schedule_program_name">${displayName}</div>`;
            const nameHTML = isRestricted ? urlNameP : `<div class="schedule_program_name"><a href="${url}" target="_blank">${displayName}</a></div>`;
            const baseHostData = Array.isArray(data.host) ? data.host.join(', ') : (data.host || "");
            const baseHostDataOS = data.only_the_schedule_hosts ? "" : baseHostData;
            const baseHostProgram = Array.isArray(p.host) ? p.host.join(', ') : (p.host || "");
            const baseHosts = p.schedule_only_host ? "" : baseHostDataOS;
            ...
            el.innerHTML = `
               ...
               <div class="schedule_program_content">
                   ...
                   ${Array.isArray(p.subitems) && p.subitems.length ? `
                       <div class="schedule_program_subs">
                           <ul>
                               ${p.subitems.map(sub => `
                                   <li>
                                       ${sub.hour ? `<span class="schedule_program_subs_hour">
                                           ${escapeHTML(sub.hour)}</span> ` : ''}
                                       ${sub.name ? `<span class="schedule_program_subs_name">
                                           ${sub.url ? `<a href="${escapeHTML(sub.url)}" target="_blank" rel="noopener noreferrer">${escapeHTML(sub.name)}</a>` : escapeHTML(sub.name)}</span>` : ''}
                                       ${sub.host ? ` - <span class="schedule_program_subs_host">
                                           ${escapeHTML(sub.host)}</span>` : ''}
                                   </li>
                               `).join('')}
                           </ul>
                       </div>
...
function renderSDetails() {
    ...
    activeDetailBlock.schedule.filter(p => {
        ...
        const nameHTML = url ? `<div class="schedule_detail_name" style="cursor:pointer;"><a href="${url}" target="_blank">${name}</a></div>` : `<div class="schedule_detail_name">${name}</div>`;
        els.innerHTML = `
            ${nameHTML}
...
function renderPrograms() {
    ...
    const getHostForProgram = (p) => {
        ...
        const occurrencesHost = [...new Set(activeOccurrences.flatMap(osch => {
            if (!osch.active || !osch.host) return [];
            return Array.isArray(osch.host) ? osch.host : [osch.host];
        }).map(h => h.trim()).filter(Boolean))];
        const baseHost = Array.isArray(p.host) ? p.host.join(", ") : (p.host || "");
        return p.only_the_schedule_hosts ? (occurrencesHost.length > 0 ? occurrencesHost.join(', ') : "") : baseHost;
    };
    // Mapujemy programy i od razu przypisujemy im wyliczonego hosta, 
    // aby nie liczyć tego dwukrotnie w filter i forEach
    const filteredPrograms = PROGRAMS.map(p => ({
        ...p,
        calculatedHost: getHostForProgram(p)
    })).filter(p => {
        // Podstawowe filtry widoczności
        if (p.hide_in_program || p.hide_in_schedule || p.private || p.delete || p.hide_only_information_schedule) return false;
        // Filtr stacji
    ...
    filteredPrograms.forEach(p => {
        ...
        el.innerHTML = `
            <div class="program_list_cover${showArchive ? ' archive' : ''}">
                <a href="${url}" target="_blank">${showArchive ? `<div class="archived">archiwalny</div>` : ''}${thumbnailHTML}</a>
            </div>
...
function ButtonsSites(s) {
    ...
    const butWebLive = getBtn(s.button_web_live || CONFIG.button_web_live, `<a target="_blank" href="${s.button_web_live || CONFIG.button_web_live}" class="btn-link"><i class="fa-solid fa-tower-broadcast"></i> Słuchaj na YT</a>`);
    const butWebRadioOnline = getBtn(s.button_web_radioonline || CONFIG.button_web_radioonline, `<a target="_blank" href="${s.button_web_radioonline || CONFIG.button_web_radioonline}" class="btn-link">💡 Online</a>`);
    const butWebSite = getBtn(s.button_web_site || CONFIG.button_web_site, `<a target="_blank" href="${s.button_web_site || CONFIG.button_web_site}" class="btn-link">🌐 Strona</a>`);
    const butWebOtherSite = getBtn(s.button_web_othersite_url || CONFIG.button_web_othersite_url, `<a target="_blank" href="${s.button_web_othersite_url || CONFIG.button_web_othersite_url}" class="btn-link">🌐 Strona (${s.button_web_othersite_name || CONFIG.button_web_othersite_name})</a>`);
    const butWebPrograms = getBtn(s.button_web_programs || CONFIG.button_web_programs, `<a target="_blank" href="${s.button_web_programs || CONFIG.button_web_programs}" class="btn-link">📻 Programy</a>`);
    const butWebPodcasts = getBtn(s.button_web_podcast || CONFIG.button_web_podcast, `<a target="_blank" href="${s.button_web_podcast || CONFIG.button_web_podcast}" class="btn-link"><i class="fa-solid fa-podcast"></i> Podcasty</a>`);
    const butWebPlayer = getBtn(s.button_web_player || CONFIG.button_web_player, `<a target="_blank" href="${s.button_web_player || CONFIG.button_web_player}" class="btn-link">► Player</a>`);
    const butWebSchedule = getBtn(s.button_web_schedule || CONFIG.button_web_schedule, `<a target="_blank" href="${s.button_web_schedule || CONFIG.button_web_schedule}" class="btn-link">📅 Ramówka</a>`);
    const butMediaSite = getBtn(s.button_media_site || CONFIG.button_media_site, `<a target="_blank" href="${s.button_media_site || CONFIG.button_media_site}" class="btn-link accent"><i class="fa-solid fa-photo-film"></i> Media</a>`);
    const butMediaOtherRadio = getBtn(s.button_media_otherradio_url || CONFIG.button_media_otherradio_url, `<a target="_blank" href="${s.button_media_otherradio_url || CONFIG.button_media_otherradio_url}" class="btn-link accent"><i class="fa-solid fa-radio"></i> ${s.button_media_otherradio_name || CONFIG.button_media_otherradio_name}</a>`);
    container.innerHTML = `
        ${butWebLive}
        ${butWebRadioOnline}
        ${butWebSite}
...
function updateStationUI(s) {
    ...
    // 2. Warunki logiczne
    const disableAllSchedule = (s.disable_detail_schedule && s.disable_schedule) || (CONFIG.disable_detail_schedule && CONFIG.disable_schedule) || s.disable_content_schedule || s.radio_listen || CONFIG.disable_content_schedule;
...
function AudioPlayer(url) {
    ...
    const sm = url.slice(0, 7) === 'http://' ? 'https://cors.krdrtradio.workers.dev/?url=' + encodeURIComponent(url) : url;
...
function scheduleCurrent(scheduleApp) {
    ...
    if (typeof window[fn] === "function") {
        if (Array.isArray(args)) window[fn](...args);
        else window[fn](args);
    }
}
// =====================
...
function init() {
    ...
    renderPrograms();
    updateOnAirStatus();
    document.getElementById("categoryFilter")?.addEventListener("change", renderPrograms);
    document.getElementById("searchInput")?.addEventListener("input", renderPrograms);
    document.getElementById("AllProgramsArchive")?.addEventListener("change", renderPrograms);
    setInterval(() => {
        const now = NowZone();
