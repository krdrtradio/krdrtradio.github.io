...
function renderPeoples() {
    ...
    const HTMLStripper = (str) => str ? str.replace(/<\/?[^>]+(>|$)/g, "").replace(/\n/g, "") : "";
    container.innerHTML = "";
    const peoples = PEOPLES.filter(p => {
        if (p.hide_in_people || p.private || p.delete) return false;
    ...
    peoples.forEach(p => {
        ...
        getDisplayleaders(p.name, SITE_ID).then(leaders => {
            ...
            row.querySelector(".leaders-box").innerHTML = (p.leaders ? `<div><span class="podcast_list_host_cat">Prowadzi:</span> ${Array.isArray(p.leaders) ? escapeHTML(p.leaders.join(", ")) : escapeHTML(p.leaders)}</div>` : "") + (leaders.length ? `<div><span class="podcast_list_host_cat">Audycje:</span> ` + leaders.map(item => `<a href="${item.url_immediately || item.target_url}" target="_blank">${escapeHTML(item.name)}</a>`).join(", ") + `</div>` : "");
        });
    });
}
// =====================
...
function init() {
    renderPodcasts();
    renderPeoples();
    document.getElementById("categoryFilter")?.addEventListener("change", renderPodcasts);
    document.getElementById("people_categoryFilter")?.addEventListener("change", renderPeoples);
    document.getElementById("searchInput")?.addEventListener("input", renderPodcasts);
    document.getElementById("people_searchInput")?.addEventListener("input", renderPeoples);
    document.getElementById("AllProgramsArchive")?.addEventListener("change", renderPodcasts);
}
