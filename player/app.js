...
const currentStationFrequency = document.getElementById("currentStationFrequency");
const reloadBtn = document.getElementById("reloadBtn");
const downloadBtn = document.getElementById("downloadBtn");
const streamContainer = document.getElementById("streamContainer");
const streamSelect = document.getElementById("streamSelect");
const resultCurrentProgram = document.getElementById("resultCurrentProgram");
...
function play(station, element) {
    ...
    currentStationText.textContent = "Teraz grasz: " + station.name;
...
function createStreamOptions(station) {
    ...
    if (station.streams && Object.keys(station.streams).length) {
...
function updateStationLinks(station) {
    if (station.site) {
        resultSite.innerHTML = `<a href="${station.site}" target="_blank">
            Przejdź na witrynę
        </a>`;
    } else {
    ...
    if (station.goto_site) {
        resultGoToRadios.innerHTML = `<a href="${station.goto_site}" target="_blank">
    ...
    if (station.goto_media) {
        resultGoToMedia.innerHTML = `<a href="${station.goto_media}" target="_blank">
...
function startMetadata(station) {
    ...
    const updateTrack = () => {
        if (!station.playlist || !station.playlist.function) {
            resultTrack.textContent = "";
            return;
        }
        const fn = station.playlist.function;
        const arg = station.playlist.argument;
...
async function loadSchedule(schedule) {
    ...
    const url = `https://current-program.krdrtradio.workers.dev/?si=${schedule.site}&st=${schedule.station}`;
    try {
        ...
        if (!container) return;
        if (data.success) {
            container.innerHTML = `<small>Na antenie:</small><br>${data.name}${data.host ? "<br><small>" + data.host + "</small>": ""}`;
        } else {
            container.innerHTML = "";
        }
    } catch (e) {
...
/* RELOAD */
reloadBtn.onclick = () => {
    if (currentStation && currentElement) {
