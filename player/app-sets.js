...
function setActive(i) {
    ...
    document.querySelectorAll(".station-item")[i].classList.add("active");
}
/* RENDER DJ */
function renderArtist(i) {
    ...
    dj.tracks.forEach((t, idx) => {
        ...
        if (t.audio) {
            html += `
                <button onclick="playTrack('https://cors.krdrtradio.workers.dev/?url=${encodeURIComponent(t.track)}', ${i}, ${idx})">
                    ▶ Play
                </button>
            `;
        } else {
