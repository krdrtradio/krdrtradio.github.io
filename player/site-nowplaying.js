...
async function getNowPlayingGrupaZPR(stationId) {
    ...
    try {
        ...
        if (data.current !== null) {
            ...
            container.innerHTML = `<small>Teraz gramy:</small><br>${SongToLink(`${artist} - ${title}`)}`;
        } else {
...
function renderProgramGrupaZPR(program) {
    const container = document.getElementById('resultCurrentProgram');
    if (!container) return;
    if (!program) {
    ...
    const image = program.thumbnail_uri ? `<img src="${program.thumbnail_uri}"
...
async function getNowPlayingAgora(stationId) {
    ...
    try {
        ...
        if (track && track.artist_name && track.song_title) {
            container.innerHTML = `<small>Teraz gramy:</small><br>${SongToLink(`${track.artist_name} - ${track.song_title}`)}`;
        } else {
...
async function getNowPlayingGrupaRMF(stationId) {
    ...
    try {
        ...
        if (utwor) {
            ...
            container.innerHTML = `<small>Teraz gramy:</small><br>${SongToLink(tekst)}`; // Aktualnie w RMF FM
...
async function getNowPlayingRadio(stationId) {
    ...
    try {
        ...
        if (data && data.length > 0) {
            ...
            container.innerHTML = `<small>Teraz gramy:</small><br>${SongToLink(currentTrack)}`;
        } else {
...
async function getNowPlayingPlaylist(stationId) {
    ...
    try {
        ...
        if (songTitle) {
            container.innerHTML = `<small>Teraz gramy:</small><br>${SongToLink(songTitle)}`; // Ostatnio grany utwór:
        } else {
...
async function getNowPlayingOpenFm(stationId) {
    ...
    try {
        ...
        if (currentSong) {
            if (container) container.innerHTML = `<small>Teraz gramy:</small><br>${SongToLink(currentSong)}`;
            return currentSong;
        } else {
...
async function getPlanetaFMSong() {
    try {
        ...
        container.innerHTML = `<small>Teraz gramy:</small><br>${SongToLink(songs)}`; // Aktualny utwór:
    } catch (error) {
        if (error instanceof TypeError) {
...
async function getTomorrowlandSong(stationId = 'main') {
    ...
    try {
        ...
        if (container) {
            container.innerHTML = `<small>Teraz gramy:</small><br>${SongToLink(formatToTitleCase(formattedTrack))}`;
        }
...
async function getNowPlayingRevma(stationId) {
    ...
    try {
        ...
        if (result === "" || result === null) {
            ...
            container.innerHTML = `<small>Teraz gramy:</small><br>${SongToLink(formatToTitleCase(result))}`;
        }
        return result;
    } catch (error) {
        if (error instanceof TypeError) {
    ...
    return new Promise((resolve, reject) => {
        ...
        eventSource.onmessage = (event) => {
            try {
                ...
                container.innerHTML = `<small>Teraz gramy:</small><br>${SongToLink(formatToTitleCase(data.streamTitle))}`;
            } catch (error) {
                eventSource.close();
...
async function getNowPlayingOnlineRadioBox(stationId) {
    try {
        ...
        if (title === "" || title === null) {
            ...
            container.innerHTML = `<small>Teraz gramy:</small><br>${SongToLink(formatToTitleCase(title))}`;
        }
    } catch (error) {
        if (error instanceof TypeError) {
            console.error(error);
...
async function getNowPlayingBounce() {
    ...
    try {
        ...
        if (result === "" || result === null) {
            ...
            container.innerHTML = `<small>Teraz gramy:</small><br>${SongToLink(result)}`;
        }
        return result;
    } catch (error) {
        if (error instanceof TypeError) {
